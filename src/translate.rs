...
pub async fn badtranslate(
    ...
    current_translation = translate(&current_translation.to_string(),Some("auto"),Some("en"))
			.await.unwrap(); //bt_beautify_response(reqwest::get(format!("https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=en&dt=t&q={}", utf8_percent_encode(input, NON_ALPHANUMERIC))).await?.json().await?).unwrap();
